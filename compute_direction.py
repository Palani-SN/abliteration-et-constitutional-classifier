...
class DirectionComputer:
    ...
    def __init__(self, dataset_path="dataset/", activations_dir="activations",
                 out_path=None, sig_path=None, mult_factor=0.6, top_n=None,
                 sig_band=6, sig_top_n=3, sig_bottom_n=3):

        self.activations_dir = Path(activations_dir)
        self.out_path = Path(out_path) if out_path else self.activations_dir / "direction.pt"
        self.sig_path = Path(sig_path) if sig_path else self.activations_dir / "signature.pt"
