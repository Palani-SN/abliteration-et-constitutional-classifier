...
class Abliterator:
    ...
    def _text_backbone(self):
        # Plain decoder-only models expose embed_tokens/layers directly on
        # model.model. Some multimodal-capable checkpoints (e.g. Gemma 3
        # 4B/12B, loaded here in text-only mode) nest the text decoder one
        # level deeper under model.model.language_model instead — the vision
        # tower lives alongside it at the same level as embed_tokens/layers
        # would otherwise be.
        ...
        raise AttributeError(
            f"Could not locate the text decoder backbone: {type(base).__name__} "
            f"has neither 'embed_tokens' nor a 'language_model' submodule."
        )
    ...
    def enable_ablation(self):
        ...
        self._hook_handles = [backbone.embed_tokens.register_forward_hook(hook)]
...
if __name__ == "__main__":
    ...
    abliterator = Abliterator(
        ...
        direction_path=f"observations/{model_key}/direction.pt",
        top_n=3,
